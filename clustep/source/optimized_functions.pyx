...
cdef double cumulative_mass(double r, double M_gas, double a_gas, double M_dm,
                            double a_dm, double gamma_gas, double gamma_dm):
  cdef double gas_mass, dm_mass
  gas_mass = M_gas * pow((r/(r+a_gas)), 3-gamma_gas)
  dm_mass = M_dm * pow((r/(r+a_dm)), 3-gamma_dm)
  return gas_mass + dm_mass


def T_integrand(double t, double M_gas, double a_gas, double M_dm,
                double a_dm, double gamma_gas, double gamma_dm):
  return (G*cumulative_mass(t, M_gas, a_gas, M_dm, a_dm, gamma_gas, gamma_dm) *
      dehnen_density(t, M_gas, a_gas, gamma_gas)) / (t*t)
